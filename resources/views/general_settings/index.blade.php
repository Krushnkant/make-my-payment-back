                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="forms-sample" action="{{ route('general_settings.store')  }}" method="post">
                            ...
                            <div class="form-group">
                                <label for="mobileAds">Mobile Ads</label>
                                <label class="switch">
                                    <input onchange="update_status_ads(this)" type="checkbox"  value="{{ $settings->is_ads_active }}" id="is_ads_active" name="is_ads_active" <?php if($settings->is_ads_active == 1){ echo "checked";} ?>>
...
<script>
    function update_status_ads(el){
        if(el.checked){
            var status = 1;
        } else {
            var status = 0;
        }
        jQuery("#is_ads_active").val(status);
    }
