...
class VersionController extends Controller
{
    ...
    public function compareData(Request $request){
        ...
        //   // if()
        //   $response['data'] = ['status' => true];
        // } elseif(!$versionDataOn && !$versionDataOff) {
        //   $response['data'] = ['status' => true];
        // } else {
        ...
        $liveVersionData = VersionModel::where('device_type', '=', $deviceType)->Where('status', '=', '1')->first();
        $versionData = VersionModel::where('device_type', '=', $deviceType)->Where('version_no', '=', $versionNo)->first();
        ...
        if(($liveVersionData && $liveVersionData->version_no == $versionNo && $versionData->status == '1') || !$liveVersionData){
          $response['data'] = ['status' => true];
        }
