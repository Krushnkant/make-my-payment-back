...
class ReportController extends Controller
{
    public function userReport(Request $request)
    {
        ...
        // Fetch only required columns and group directly in the query
        $userYears = User::selectRaw('YEAR(created_at) as year, COUNT(id) as count')
                    ->groupBy('year')
                    ->orderBy('year', 'asc')
                    ->get();

        $usermcount = $userYears->pluck('count', 'year')->toArray();

        // Get all years between min and max years in the dataset
        $min = $userYears->min('year') ?? date('Y'); // Default to current year if no users
        $max = $userYears->max('year') ?? date('Y');
        ...
        for ($i = $min - 1; $i <= $max + 1; $i++) {
            $finalyear[] = 'Year ' . $i;
            $uservalue[] = $usermcount[$i] ?? 0; // Use 0 if no data for the year
        }
