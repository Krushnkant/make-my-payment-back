...
class UserController extends Controller
{
    ...
    // Update User's SMS/Call Credit after showing the Ads
    public function updateSmsCallCredit(Request $request)
    {
        try {
            $validator=Validator::make($request->all(),[
                'user_id' => 'required',
                'type' => 'required' // 1 - call / 2 - sms
            ]);
            ...
            if($validator->fails())
            {
                ...
                if(isset($user) && $user != ""){
                    ...
                    if($type == '1'){
                        $user->total_call += 1;
                        $msg = "Call Credit has been updated successfully.";
                    }
                    if($type == '2'){
                        $user->total_message += 1;
                        $msg = "SMS Credit has been updated successfully.";
                    }
                    if($user->save()){
                        return ResponseAPI(true, $msg, "","", 200);
                    }
                }
                if($user->save()){
                    return ResponseAPI(true,"User updated Succesfull", "", "", 200, 0);
