                    ...
                    if($request->type == 'CALL'){
                        ...
                        if($userdata->total_call > 0){
                            if(isset($customer->phone) && isset($customer->name) && isset($customer->balance)){
                                ...
                                return ResponseAPI(false, 'This number is registered on DND. Calls are not allowed, but SMS can still be sent under telecom regulations.', "",array(),401);
                                // Log::info('User logged in 8.', ['user_id' => $response]);
                            } else {
                                // Log::info('User logged in 6.', ['user_id' => $response]);
                                return ResponseAPI(false, 'Customer not Exist.', "",array(), 401);
                            }
                        } else {
