...
class RozerPayController extends Controller
{
    ...
    {
        Log::info('1. Order Create API - Called. '.now().' - ', ['user_id' => $request->user_id, 'package_id' => $request->package_id, 'is_test_order' => $request->is_test_order]);
        try {
            ...
            if($validator->fails()){
                Log::info('2. Order Create API - Validation Error for Required. '.now().' - ', ['order_id' => '']);
                ...
                $totalPrice_for_razorpay = ($price + $gstAmount) * 100;
                $totalPrice = $price + $gstAmount;
                $order = $api->order->create(array('receipt' => (string) $receipt, 'amount' => (string) $totalPrice_for_razorpay, 'currency' => 'INR', 'notes'=> array('name'=> $user->name,'contact'=> $user->phone)));
                ...
                if(isset($order) && !empty($order)){
                    ...
                    $transection->transection_amount = $totalPrice;
                    if($transection->save()){
                        Log::info('3. Order Create API - Success. '.now().' - ', ['order_id' => $transection->order_id]);
                        return ResponseAPI(true,"Order created successfully.", "", $transection->order_id, 200);
                    }
                }else{
                    Log::info('4. Order Create API - Failed. '.now().' - ', ['order_id' => '']);
                    return ResponseAPI(false,'Order cannot created',"",array(),401);
                }
            }
            
        } catch (\Throwable $th) {
            Log::info('5. Order Create API - Try...catch Error. '.now().' - ', ['order_id' => '']);
    ...
    {
        Log::info('1. Payment Create API - Called. '.now().' - ', ['user_id' => $request->user_id, 'razorpay_signature' => $request->razorpay_signature, 'razorpay_order_id' => $request->razorpay_order_id, 'is_test_order' => $request->is_test_order]);
        try {
            $validator=Validator::make($request->all(),[
            ...
            if($razorpay_signature == $generated_signature){
                    ...
                    if(isset($transection) && $transection != ""){
                        ...
                        if($transection->save()){
                            ...
                            if(isset($user) && $user != ""){
                                ...
                                if($user->save()){
                                    Log::info('2. Payment Create API - Success. '.now().' - ', ['order_id' => '']);
                ...
                Log::info('2. Payment Create API - razorpay signature not matched. '.now().' - ', ['order_id' => '']);
                return ResponseAPI(false,'razorpay signature not matched.',"",array(),401);
            }

        } catch (\Throwable $th) {
            Log::info('2. Payment Create API - Try...catch Error. '.now().' - ', ['order_id' => '']);
            return ResponseAPI(false,'Something Wrongs.',"",array(),401);
        }
    }
